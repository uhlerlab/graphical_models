...
class TestAncestralGraph(TestCase):
    def setUp(self):
        self.d = AncestralGraph(directed={(1, 3)}, bidirected={(3, 4)}, undirected={(1, 2)})
    ...
    #     with self.assertRaises(classes.ancestral_graph.AdjacentError) as cm:
    ...
    #     d = AncestralGraph(directed={(1, 2), (2, 3)})
    #     with self.assertRaises(classes.ancestral_graph.CycleError) as cm:
    #         d.add_directed(3, 1)
    #
    # def test_add_spouse_error(self):
    #     d = AncestralGraph(directed={(1, 2), (2, 3)})
    #     with self.assertRaises(classes.ancestral_graph.SpouseError) as cm:
    ...
    def test_msep_from_given(self):
        d = AncestralGraph(directed={(1, 2), (3, 2), (2, 4), (3, 4)})
    ...
    def test_msep(self):
        return
        # 1 -> 3 <-> 4 <- 2
        d = AncestralGraph(directed={(1, 3), (2, 4)}, bidirected={(3, 4)})
        ...
        g = rand.directed_erdos(nnodes, 1/(nnodes-1))
        print(g.arcs)
    ...
    #     d = DAG(nodes=set(range(10)), arcs={(0, 9), (1, 9), (8, 9), (2, 5)})
    ...
    def test_disc_paths(self):
        ...
        g = AncestralGraph(nodes=set(range(1, 5)), directed={(1, 2), (2, 4)}, bidirected={(3, 2), (3, 4)})
        disc_paths = g.discriminating_paths()
        self.assertEqual(disc_paths, {(1, 2, 3, 4): 'c'})

        g = AncestralGraph(nodes=set(range(1, 6)), directed={(1, 2), (2, 5), (3, 5)}, bidirected={(2, 3), (3, 4), (4, 5)})
        disc_paths = g.discriminating_paths()
        # print(disc_paths)
    ...
    def test_legitimate_mark_changes(self):
        ...
        g = AncestralGraph(directed={(0, 1), (1, 2)})
        lmcs = g.legitimate_mark_changes()
        self.assertEqual(lmcs, ({(0, 1)}, set()))

        g = AncestralGraph(directed={(2, 1), (2, 3), (3, 5)}, bidirected={(1, 3), (4, 5), (2, 4)})
        lmcs = g.legitimate_mark_changes()
        self.assertEqual(lmcs, (set(), {(1, 3), (2, 4), (3, 1)}))

    def test_ancestor_dict(self):
        g = AncestralGraph(bidirected={(0, 1)}, directed={(0, 2), (1, 3), (2, 4), (3, 4)})
    ...
    def test_fast_markov_equivalence_simple(self):
        g1 = AncestralGraph(directed={(0, 1), (1, 3)}, bidirected={(1, 2), (2, 3)})
        g2 = AncestralGraph(directed={(0, 1), (1, 3), (1, 2)}, bidirected={(2, 3)})
        g3 = AncestralGraph(directed={(0, 1), (1, 2), (1, 3), (3, 2)})
    ...
    def test_fast_markov_equivalence_all(self):
        ...
        random.seed(s)
        np.random.seed(s)
        d = directed_erdos(10, exp_nbrs=2)
