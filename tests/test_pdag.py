...
class TestDAG(TestCase):
    ...
    def test_pdag2alldags_5nodes(self):
        dag = DAG(arcs={(1, 2), (2, 3), (1, 3), (2, 4), (2, 5), (3, 5), (4, 5)})
        cpdag = dag.cpdag()
        dags = cpdag.all_dags()
        for arcs in dags:
            dag2 = DAG(arcs=set(arcs))
            cpdag2 = dag2.cpdag()
            if cpdag2 != cpdag:
    ...
    def test_shd(self):
        ...
        d1 = PDAG(arcs={(0, 1), (0, 2)})
        d2 = PDAG(arcs={(0, 1)}, edges={(0, 2)})
        self.assertEqual(d1.shd(d2), 1)
        self.assertEqual(d2.shd(d1), 1)

        d1 = PDAG(arcs={(0, 1), (0, 2)})
        d2 = PDAG(arcs={(1, 0), (2, 0)})
        self.assertEqual(d1.shd(d2), 2)
        self.assertEqual(d2.shd(d1), 2)

        d1 = PDAG(arcs={(0, 1), (0, 2)})
        d2 = PDAG(edges={(1, 0), (2, 0)})
        self.assertEqual(d1.shd(d2), 2)
        self.assertEqual(d2.shd(d1), 2)

    def test_to_complete_pdag(self):
        g = PDAG(arcs={(0, 1), (0, 2), (3, 0)}, edges={(1, 2), (1, 3), (2, 3)})
        g.to_complete_pdag()
        g_comp = PDAG(arcs={(0, 1), (0, 2), (3, 0), (3, 1), (3, 2)}, edges={(1, 2)})
        self.assertEqual(g, g_comp)

        g = PDAG(arcs={(0, 1), (0, 2), (0, 3)}, edges={(1, 2), (1, 3), (2, 3)})
        g.to_complete_pdag()
        g_comp = PDAG(arcs={(0, 1), (0, 2), (0, 3)}, edges={(1, 2), (1, 3), (2, 3)})
        self.assertEqual(g, g_comp)

        g = PDAG(arcs={(1, 0), (2, 0), (3, 0)}, edges={(1, 2), (1, 3), (2, 3)})
        g.to_complete_pdag()
        g_comp = PDAG(arcs={(1, 0), (2, 0), (3, 0)}, edges={(1, 2), (1, 3), (2, 3)})
        self.assertEqual(g, g_comp)
